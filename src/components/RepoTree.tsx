...
const RepoTree: React.FC<RepoTreeProps> = ({ owner, repo, repoMeta }) => {
  ...
  const fetchTree = async (branch: string) => {
    ...
    try {
      ...
      setError('Failed to load repo tree - ' + (err instanceof Error ? err.message : 'Unknown error'));
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center py-32 min-h-screen bg-gradient-to-br from-white to-gray-100">
      <div className="w-14 h-14 border-4 border-black border-t-white rounded-full animate-spin"></div>
