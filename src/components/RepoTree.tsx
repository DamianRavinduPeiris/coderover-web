...
import { fetchRepoTree } from '../util/repoTreeApi';
        ...
        <div className="mb-6 flex flex-col gap-2">
          ...
          {repoMeta && (
            <div className="flex flex-col md:items-end md:w-full gap-1 text-gray-700 text-sm bg-white rounded px-0 py-2 border-b border-gray-200 mb-2">
              <span className="font-semibold text-xl text-blue-900">{repoMeta.name}</span>
              {repoMeta.description && <span className="text-xs text-gray-500 italic max-w-2xl whitespace-pre-line">{repoMeta.description}</span>}
              <div className="flex items-center gap-2 mt-1 flex-wrap">
                {repoMeta.private && <span className="px-2 py-1 text-xs bg-gray-100 text-gray-600 rounded border border-gray-200">Private</span>}
                {repoMeta.language && <span className="flex items-center gap-1"><div className="w-3 h-3 bg-yellow-400 rounded-full"></div><span className="font-medium text-xs text-gray-700">{repoMeta.language}</span></span>}
                <span className="flex items-center gap-1 text-xs text-gray-600"><Star className="h-4 w-4 text-yellow-500" />{repoMeta.stargazers_count}</span>
                <span className="text-xs text-gray-500">Updated <span className="font-mono">{new Date(repoMeta.updated_at).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' })}</span></span>
              </div>
            </div>
          )}
        </div>
